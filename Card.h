...
//  Data members of class Card
  ...
  public:
    Card();   //   It calls a random number generator to produce a random number between 1 - 13, and then
              //   assigns the typeString accordingly, i.e. 1 becomes an "Ace", 2 becomes a "Two",
              //   3 becomes a "Three", etc.  11 becomes a "Jack", 12 becomes a "Queen", 
              //   13 becomes a "King".  Also set the initial_value of the card.
    std::string getType() const;  // It returns the string typeString
    int getValue() const;   //  returns the int initial_value
};
